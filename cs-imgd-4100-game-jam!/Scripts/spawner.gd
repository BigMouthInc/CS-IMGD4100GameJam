...
func nearBlock(new_pos: Vector2) -> bool:
	...
	for platform in platforms.get_children():
		if platform.position.x - 20 < new_pos.x and new_pos.x < platform.position.x + 20:
			return true
		if  platform.position.y - 20 > new_pos.y and new_pos.y > platform.position.y + 20:
			return true
